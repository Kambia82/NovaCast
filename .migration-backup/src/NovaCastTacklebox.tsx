...
// NOTE: "Read Water" content is a stub. No locked spec or source content existed for
// this tab — it was added per your call to keep all 4 tabs from the source snippet.
// Flagging this as placeholder copy, not researched/verified content.
const readWaterTips = [
  {
    title: 'Drop-Offs & Ledges',
    text: 'Watch for color changes in the water — a sudden shift from light to dark often marks a ledge where depth changes sharply. Fish stage on these edges.',
  },
  {
    title: 'Current Seams',
    text: 'Where fast water meets slow water, fish sit in the slack side and ambush prey drifting through the seam.',
  },
  {
    title: 'Shade Lines',
    text: 'On bright days, the edge of shade from docks, trees, or bridges concentrates baitfish — and the predators following them.',
  },
];
...
export default function NovaCastTacklebox({ onBack, externalTacklebox, onToggleSaved }: NovaCastTackleboxProps) {
  ...
  const renderReadWater = () => (
    <div className="space-y-2">
      <div className="text-[11px] text-[#4A6878] leading-relaxed px-1 mb-1">
        Placeholder content — not yet reviewed or expanded.
      </div>
      {readWaterTips.map(tip => {
        const id = `water-${tip.title}`;
        const isOpen = openCard === id;
        return (
          <div key={id} className="bg-[#0c1822] border border-[#1A3346] rounded-2xl overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3.5 text-left"
            >
              <span className="font-semibold text-sm text-[#C8E4F0]">{tip.title}</span>
              <ChevronDown className={`w-4 h-4 text-[#4A6878] shrink-0 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
            </button>
            {isOpen && (
              <div className="px-4 pb-4 border-t border-[#1A3346] pt-3">
                <div className="text-xs text-[#A8C8D8] leading-relaxed">{tip.text}</div>
              </div>
            )}
          </div>
        );
